...
async function generateWithModelFallback(
  ...
  for (const model of CANDIDATE_MODELS) {
    try {
      ...
      console.warn(`Model ${model} temporarily unavailable (${err?.status || err?.message}), attempting alternative...`);
...
// Safety check regex according to Rule A9 #1:
// Must use \w* stems, never trailing \b on roots: /\bsuicid\w*/i, /\babus\w*/i
export function testCrisisText(text: string): boolean {
  if (!text || typeof text !== "string") return false;
  const normalized = text.toLowerCase();
  
  // Specific crisis regex patterns with \w* stems
  const crisisPatterns = [
    /\bsuicid\w*/i,                  // matches suicide, suicidal, suiciding, etc.
    /\babus\w*/i,                    // matches abuse, abused, abuses, abusing, etc.
    /\bself[- ]?harm\w*/i,           // matches self-harm, selfharm, self harming
...
app.post("/api/safety/screen", (req, res) => {
  try {
    const { text } = req.body;
    const flagged = testCrisisText(text || "");
    res.json({ flagged });
  } catch (error) {
    // Rule A9 #4: Fail closed. If safety endpoint errors, treat input as flagged
    res.json({ flagged: true });
  }
});

// 3. Ask Pooja (Chat) endpoint with verbatim prompt from A8
...
You are not a therapist. Never diagnose. If a message suggests self-harm, abuse, violence or a medical emergency, stop and trigger the safety screen instead of replying.`;
    ...
    if (previousOpenings && previousOpenings.length > 0) {
      contextNotes.push(`Recent message openings used (DO NOT repeat these openings):\n- ${previousOpenings.join("\n- ")}`);
    }
    ...
    // Contextual fallback if Gemini is offline / temporarily 503
    if (!reply) {
      isFallback = true;
    ...
    // Check if output itself triggered any crisis flag
...
app.post("/api/gemini/weekly-letter", async (req, res) => {
  try {
...
// 7. Automatic Nudge Message Generator (Rule A6b)
app.post("/api/gemini/nudge-message", async (req, res) => {
  try {
...
// 8. TTS endpoint (returns standard Web Audio synthesis fallback or Gemini TTS if requested)
app.post("/api/gemini/tts", async (req, res) => {
  try {
    ...
    // Client can fallback to browser SpeechSynthesis API gracefully
...
async function start() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Rise app server listening on port ${PORT}`);
  });
}
